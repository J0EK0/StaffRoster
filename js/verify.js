...
// ── 7月實際偏好測試資料 ──────────────────────────────────────────────
const JULY_CONSTRAINTS = {
  caimh:  { '2026-07-16':'請', '2026-07-17':'請', '2026-07-18':'請' },
  sjuan:  { '2026-07-01':'請' },
  liwj:   { '2026-07-02':'請', '2026-07-03':'請', '2026-07-29':'請' },
  lixf:   { '2026-07-10':'請', '2026-07-17':'請', '2026-07-20':'請' },
  laiyh:  {
    '2026-07-05':'請', '2026-07-06':'請', '2026-07-07':'請', '2026-07-08':'請',
    '2026-07-09':'請', '2026-07-10':'請', '2026-07-11':'請', '2026-07-12':'請',
    '2026-07-13':'請', '2026-07-14':'請', '2026-07-15':'請', '2026-07-16':'請',
    '2026-07-17':'請', '2026-07-18':'請', '2026-07-19':'請', '2026-07-20':'請',
    '2026-07-21':'請', '2026-07-22':'請', '2026-07-23':'請', '2026-07-24':'請',
    '2026-07-25':'請', '2026-07-26':'請', '2026-07-27':'請',
  },
  huangym:{ '2026-07-03':'請', '2026-07-13':'請', '2026-07-24':'請' },
  caicc:  { '2026-07-24':'請', '2026-07-25':'請', '2026-07-26':'請', '2026-07-27':'請' },
  chenye: { '2026-07-20':'請', '2026-07-21':'請' },
  linyh:  { '2026-07-06':'請' },
  qiupr:  {
    '2026-07-17':'請', '2026-07-18':'請', '2026-07-19':'請', '2026-07-20':'請',
    '2026-07-21':'請', '2026-07-22':'請', '2026-07-23':'請', '2026-07-24':'請',
    '2026-07-25':'請', '2026-07-26':'請', '2026-07-27':'請', '2026-07-28':'請',
    '2026-07-29':'請', '2026-07-30':'請', '2026-07-31':'請',
  },
};

// 僅保留 2026-07-xx 日期的平日底稿
const JULY_ROTATION_WEEKDAY = {
  _inactive: {},
...
// 僅保留 2026-07-xx 日期的假日底稿
const JULY_ROTATION_REGULAR = {
  _inactive: {},
  _order: ['linyh','huangym','chenye','huangwl','lixf','huangyy','chenyt','laiyh','caicc','qiupr','liwj','wupc','xiemq','yangyy','linyy','linzt','linc','huangwl2','zhuangcq'],
  _orderPreset: 'holiday-order-20260513-v1',
  liwj:   { '2026-07-11':'E' },
...
// 7月無國定假日，national 區只保留排序
const JULY_ROTATION_NATIONAL = {
  _inactive: {},
  _order: ['linyh','huangym','chenye','huangwl','lixf','huangyy','chenyt','laiyh','caicc','qiupr','liwj','wupc','xiemq','yangyy','linyy','linzt','linc','huangwl2','zhuangcq'],
};

const JULY_ROTATION = {
  weekday: JULY_ROTATION_WEEKDAY,
  regular: JULY_ROTATION_REGULAR,
  national: JULY_ROTATION_NATIONAL,
};

function targetedJulyPrefsTest() {
  const y = 2026;
  const m = 7;
  const days = buildMonthStructure(y, m, {});

  // init-repair 流程：先鋪底稿，再套偏好修班
  const schedule = { year: y, month: m, days, assignments: {} };
  Scheduler.initPatternDraft(schedule, staff, {}, JULY_ROTATION);
  const draftCoverage = typeCount(Validator.validate(schedule, staff), ['short-coverage', 'duplicate-coverage']);
  const snapshot = snapshotHolidayPattern(schedule, days);
  Scheduler.repairSchedule(schedule, staff, JULY_CONSTRAINTS);
  ...
  process.stdout.write('\n[2026-07 with prefs (init-repair)]\n');
  check('底稿每日特殊班缺/重複 0', draftCoverage.length === 0,
    draftCoverage.map(e => e.msg).join('; '));
  check('國月配額錯誤 0', guo.length === 0, guo.map(e => e.msg).join('; '));
  check('每日特殊班缺/重複 0', coverage.length === 0, coverage.map(e => e.msg).join('; '));
  process.stdout.write(`  validator 殘餘錯誤統計: weekly=${weekly.length}, hard=${hard.length}, holiday-hard=${holidayLockedHard.length}\n`);
  check('假日底稿修班後不變', changed.length === 0,
    changed.slice(0, 5).map(([key]) => key).join('; '));
}
